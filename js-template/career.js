...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  const res = await fetch("career.json"); //fetch関数を使ってdata.jsonを取得している
  const obj = await res.json(); //取得したレスポンスをJSON形式で解析し、resからobjにJSONデータを格納している
  const career = obj.care; //obj内のlistに含まれるデータをdataに格納している
  console.log(career);
  /*ウェブページがロードされたらサーバーから取得したJSON形式のデータを使って動的なコンテンツを生成するための処理を行う*/

  document
    .querySelectorAll("span.unread")
    .forEach((el) => (el.textContent = career.length)); //span要素がunreadの要素を選択し。その要素のテキストコンテンツをdata.lengthに設定
  /*未読の件数を表示するためにページ内のすべての未読要素に未読データの長さを設定している*/

  const career_list = document.querySelector("div#career_list");
  ...
  for (const item of career) {
    ...
    career_list.appendChild(record); //record要素をinfo_listに追加
  }
});
